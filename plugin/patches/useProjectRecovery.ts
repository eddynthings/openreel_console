...
export function useProjectRecovery() {
  // When the OpenReel Console bridge has a persisted project, it handles session
  // restore itself — skip the recovery dialog to avoid competing with the bridge.
  const bridgeManaged = !!localStorage.getItem("openreel_console_project");

  const [state, setState] = useState<RecoveryState>({
    isChecking: !bridgeManaged,
    availableSaves: [],
    showDialog: false,
  });
